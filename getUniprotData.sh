...
for gene in $( < ./listOfGenes.tsv ) ; 
    do  wget --no-verbose --output-document=uniprot/proteins/${gene}.tsv http://www.uniprot.org/uniprot/\?query=${gene}\&sort=score\&columns=id,entry%20name,protein%20names,reviewed,genes\(PREFERRED\)\&format=tab ; done

rm -f ./listofProteins.tsv

for file in uniprot/proteins/*.tsv;
